...
public class CropImage {
    ...
    public CropImage(Context context, CropImageView imageView, Handler handler, int width, int height, String picPath) {
        mContext = context;
        mImageView = imageView;
        mImageView.setCropImage(this);
        mHandler = handler;
...
//        Rect dr = mCrop.mDrawRect;
        int width = mWidth;//dr.width(); //图片截取的宽高
        int height = mHeight;//dr.height();
        Bitmap croppedImage = Bitmap.createBitmap(width, height, Bitmap.Config.RGB_565);//Bitmap.Config.ARGB_8888
        {
            Canvas canvas = new Canvas(croppedImage);
            Rect dstRect = new Rect(0, 0, width, height);
            canvas.drawBitmap(bm, r, dstRect, null);
...
//        String str = uri_index + Constants.App.pic_cache_name + ".jpg";
//        String path = FILE_LOCAL + str;
//        String path = PictureUtil.getImageUri(uri_index).getPath();
        try {
            FileOutputStream fos = new FileOutputStream(mPicPath);
